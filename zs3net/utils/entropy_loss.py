...
class EntropyLoss(nn.Module):
    ...
    def forward(self, v):
        ...
        if self.weight is None:
            if not self.hardneg:
                loss = -torch.sum(torch.mul(v, torch.log2(v+1e-30)))/(n*h*w*np.log2(c))
            else:
                entp = -torch.sum(torch.mul(v, torch.log2(v+1e-30)), dim=1)/np.log2(c)
                thres = torch.topk(entp.reshape((-1,)), int(h*w*self.hardnegPer))[-1]   # determine kth largest threshold
                mask = (entp > thres).float() * 1
                entp_masked = entp*mask
                loss = torch.sum(entp_masked)/(h*w)
        else:
            loss = -torch.sum(torch.mul(torch.mul(v,self.weight), torch.log2(v+1e-30)))/(h*w*np.log2(c))
