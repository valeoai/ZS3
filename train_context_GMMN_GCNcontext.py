...
class Trainer(object):
    ...
    def training(self, epoch, args):
        ...
        for i, sample in enumerate(tbar):
            if len(sample['image']) > 1:
                image, target, embedding = sample['image'], sample['label'], sample['label_emb']
                if self.args.cuda:
                    image, target, embedding = image.cuda(), target.cuda(), embedding.cuda()
                self.scheduler(self.optimizer, i, epoch, self.best_pred)
                # ===================real feature extraction=====================
                with torch.no_grad():
                    real_features = self.model.module.forward_before_class_prediction(image)
    ...
    def validation(self, epoch, args):
        ...
        print("Seen: Acc:{}, Acc_class:{}, mIoU:{}, fwIoU: {}".format(Acc_seen, Acc_class_seen, mIoU_seen, FWIoU_seen))
        print("Unseen: Acc:{}, Acc_class:{}, mIoU:{}, fwIoU: {}".format(Acc_unseen, Acc_class_unseen, mIoU_unseen,
                                                                        FWIoU_unseen))

        for class_name, acc_value, mIoU_value in zip(class_names, Acc_class_by_class, mIoU_by_class):
...
def main():
    ...
    parser.add_argument('--resume', type=str,
                        default='/home/docker_user/shared/mbucher/zs_segmentation/checkpoint_pascal_context_2_unseen.pth.tar',
    ...
    # unseen_classes_idx_metric = [5, 18, 3, 45, 54, 20, 35, 14]
    ...
    parser.add_argument('--GCN_weight', type=float, default=1.0, help='GCN context weight')
    parser.add_argument('--checkname', type=str,
                        default='gmmn_context_w2c300_linear_weighted100_hs256_2_unseen_withGCNcontext')
