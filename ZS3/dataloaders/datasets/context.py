...
class ContextSegmentation(Dataset):
    ...
    def __init__(self,
        ...
        self._image_dir = os.path.join(self._base_dir, 'pascal/VOCdevkit/VOC2012/JPEGImages')
        ...
        self.labels_459 = [label.decode().replace(' ', '') for idx, label in np.genfromtxt(osp.join(self._base_dir, 'full_annotations/labels.txt'), delimiter=':', dtype=None)]
        self.labels_59 = [label.decode().replace(' ', '') for idx, label in np.genfromtxt(osp.join(self._base_dir, 'classes-59.txt'), delimiter=':', dtype=None)]
        ...
        # Display stats
        print('(pascal) Number of images in {}: {:d}, {:d} deleted'.format(split, len(self.images),  len(lines)- len(self.images)))
    ...
    def __getitem__(self, index):
        ...
        if self.weak_label:
            ...
            if has_unseen_class:
                _target = Image.open('weak_label_context_10_unseen_top_by_image_75.0/pascal/'+(self.categories[index].split('/'))[-1].split('.')[0]+'.jpg')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--crop-size', type=int, default=312,
    ...
    # optimizer params
    # PASCAL VOC
    parser.add_argument('--lr', type=float, default=0.007, metavar='LR', help='learning rate (default: auto)')
    ...
    parser.add_argument('--checkname', type=str, default='gmmn_pascal_w2c300_linear_weighted100_hs256_context_aware')
    ...
    parser.add_argument('--real_seen_features', type=bool, default=False, help='real features for seen classes')
    parser.add_argument('--load_embedding', type=str, default='w2c',
