...
class Saver(object):
    ...
    def save_checkpoint(self, state, is_best, filename='checkpoint.pth.tar', generator_state=None):
            ...
            if self.runs:
                ...
                for run in self.runs:
                    ...
                    if os.path.exists(path):
                        ...
                        continue
                shutil.copyfile(filename, os.path.join(self.experiment_dir, str(state['epoch'])+'_model.pth.tar'))
                shutil.copyfile(filename_generator, os.path.join(self.experiment_dir, str(state['epoch']) + '_generator.pth.tar'))
            else:
                shutil.copyfile(filename, os.path.join(self.experiment_dir, str(state['epoch'])+'_model.pth.tar'))
